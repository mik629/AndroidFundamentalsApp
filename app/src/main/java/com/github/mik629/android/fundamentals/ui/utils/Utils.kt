...
inline fun buildGlideRequest(context: Context): GlideRequest<Drawable> =
    addStandardGlideOptions(glideRequest = GlideApp.with(context).asDrawable())

inline fun buildGlideRequest(fragment: Fragment): GlideRequest<Drawable> =
    addStandardGlideOptions(glideRequest = GlideApp.with(fragment).asDrawable())

inline fun addStandardGlideOptions(glideRequest: GlideRequest<Drawable>): GlideRequest<Drawable> =
    glideRequest.thumbnail(0.1f)
